...
inline void lbfgsbactive(const int& n, /// length of all vectors
	const real* l,	/// lower constraint vector of size n
	const real* u,	/// upper constraint vector of size n
	const int* nbd,	/// number of constraints vector of size n for each input value: can be 0 for no constraint, 1 for lower only, 2 for upper and lower and 3 for upper only
	real* x);		/// input vector of size n
	 ...
	 void(*funcgrad)(real* x, real& f, real* g, const cudaStream_t& stream))
{
	...
	for(int i = 0; i < MAX_STREAM; i++)
		checkCudaErrors(cudaStreamCreate(streamPool + i));
    ...
    nfgv = 1;
    lbfgsbprojgr(n, l, u, nbd, x, g, buf_n_r, sbgnrm_h, sbgnrm_d, NULL); // find the bounds of the active set
    ...
    while(true)
    {
		...
		//printf("lbfgs iter: %d  change %.3g  stopping threshold: %.3g\n", iter, fold - f, epsf * ddum);
		if( fold - f <= epsf * ddum )
		{
			info = 1;
			break;
