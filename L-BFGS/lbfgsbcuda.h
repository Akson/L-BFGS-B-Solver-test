...
#define EPSG 5E-32 // was 5e-16
#define EPSF 5E-32 // was 5e-16
#define EPSX 5E-32 // was 5e-16
#define MAXITS 1000
#else
typedef float real;
...
namespace lbfgsbcuda {
	...
	inline void CheckBuffer(const real* q, int stride, int total) {
		...
		if (err != cudaSuccess) 
		{
			printf("!! Cuda error on entry CheckBuffer: %s\n", cudaGetErrorString(err));
		}
		...
		//checkCudaErrors(cudaMemcpy(hq, q, total * sizeof(real), cudaMemcpyDeviceToHost));
		...
		if (err != cudaSuccess) 
		{
			printf("!! Cuda error in CheckBuffer: %s\n", cudaGetErrorString(err));
		}
/*
...
/*		delete[] pBufStr;*/
	...
	inline void CheckBuffer2(const real* q, int stride, int total) {

		cudaError_t err = cudaGetLastError();
		if (err != cudaSuccess) 
		{
			printf("!! Cuda error on entry CheckBuffer2: %s\n", cudaGetErrorString(err));
		}
		...
		if (err != cudaSuccess) 
		{
			printf("!! Cuda error in CheckBuffer2: %s\n", cudaGetErrorString(err));
		}
/*
		char* pBufStr = new char[30 * wh];
		pBufStr[0] = '\0';
...
/*		delete[] pBufStr;*/
	...
	inline void CheckBuffer_int(int* q, int stride, int total) {
		...
		checkCudaErrors(cudaMemcpy(hq, q, total * sizeof(int), cudaMemcpyDeviceToHost));
/*
