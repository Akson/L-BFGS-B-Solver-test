...
namespace lbfgsbcuda {
	namespace subsm {
		...
		)
		{
			...
			// This kernel implements the block of code in lbfgsb.f from lines 3290 to 3317
			const int i = blockIdx.x * blockDim.x + threadIdx.x;
			const int tid = threadIdx.x;
			...
			if(i < nsub) {
				...
				if(nbdi != 0) {
					...
					else if( dk > 0 && nbdi >= 2 )
					{
						...
						{
							mySum = minr(1.0, temp2 / dk); // line 3309
						}
					}
				}
			}
			...
			)
		{
			...
			x[i] = salpha[0] * di + xi; // Line 3332 in lbfgsb.f
		}
...
/*
			...
			// Lines 3330 to 3333 in lbfgsb.f
